...
const tokenAmount = (value) => {
  try {
    const amount = Number(ethers.formatUnits(BigInt(value || 0), 18));
    if (!Number.isFinite(amount)) return "—";
...
function Logo({ compact = false }) {
  return <a className={`brand ${compact ? "brand-compact" : ""}`} href="#/explore" aria-label="Proto home"><img src="/proto-mark.png" alt="" className="brand-mark" /><span className="brand-wordmark"><img src="/proto-wordmark-cropped.png" alt="Proto" /></span></a>;
}
...
function TradeActivity({ trades = [], symbol }) {
...
function Trade({ tokenAddress, txClient, onOpenProfile, onConnect }) {
  ...
  useEffect(() => { let active = true; const load = async () => { const item = await fetchToken(tokenAddress); if (active) setToken(item); }; load(); const timer = setInterval(load, 15000); return () => { active = false; clearInterval(timer); }; }, [tokenAddress]);
  useEffect(() => {
    let active = true;
    const loadBalance = async () => {
      if (!txClient || !token || mode !== "sell") return setWalletBalance(0n);
      try {
        const account = await txClient.account();
        const contract = new ethers.Contract(token.token, ["function balanceOf(address) view returns (uint256)"], txClient.provider);
        const balance = await contract.balanceOf(account);
        if (active) setWalletBalance(BigInt(balance));
      } catch { if (active) setWalletBalance(0n); }
    };
    loadBalance();
    return () => { active = false; };
  }, [token, txClient, mode]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (mode === "buy") {
        ...
        await tx.wait();
        setWalletBalance((current) => current > tokenIn ? current - tokenIn : 0n);
      }
...
function CreatorFeeClaim({ address, txClient, onConnect }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try { const amount = await reader.getCreatorFees(address); if (active) setClaimable(amount); }
      catch { if (active) setClaimable(null); }
    };
    load();
  ...
  const claim = async () => {
    ...
    setBusy(true); setMessage("");
    try {
      const tx = await txClient.claimCreatorFees(address);
      await tx.wait();
      setClaimable(0n);
      setMessage("Creator fees claimed.");
    } catch (error) { setMessage(error.shortMessage || error.message || "Claim failed"); }
    finally { setBusy(false); }
  };
  const hasClaimable = claimable != null && claimable > 0n;
  const amountLabel = claimable == null ? "—" : `${Number(ethers.formatEther(claimable)).toLocaleString(undefined, { maximumFractionDigits: 5 })} ETH`;
  return <section className="profile-section creator-fee-panel"><div className="section-heading"><h2>Creator fees</h2><span>35% of trading fees</span></div><div className="fee-card"><div><span>Claimable</span><strong>{amountLabel}</strong></div><button className="primary-button" onClick={claim} disabled={busy || !hasClaimable}>{busy ? "Claiming…" : txClient ? "Claim creator fees" : "Connect wallet to claim"}</button></div>{message && <div className="form-message">{message}</div>}</section>;
}

function Profile({ viewAddress, connectedAddress, txClient, onConnect, onOpen, onEditProfile }) {
  const [profile, setProfile] = useState(null); const user = viewAddress || "0x533B22BbB18A1f4dd7B6202bcaF83c69C76E055d";
  const isOwnProfile = Boolean(connectedAddress) && user.toLowerCase() === connectedAddress.toLowerCase();
  useEffect(() => { let active = true; setProfile(null); fetchProfile(user).then((item) => { const saved = isOwnProfile ? JSON.parse(localStorage.getItem("proto-profile") || "null") : null; if (active) setProfile(saved ? { ...item, ...saved, address: user } : item); }); return () => { active = false; }; }, [user, isOwnProfile]);
  if (!profile) return <main className="page loading">Loading profile…</main>;
  return <main className="page profile-page"><section className="profile-header"><div className="profile-avatar"><img src={profile.avatar || "/proto-mark.png"} alt="" /></div><div className="profile-copy"><div className="eyebrow">PROTO PROFILE</div><h1>{profile.name || "Unnamed builder"}</h1><button className="address-link" onClick={() => navigator.clipboard?.writeText(profile.address)}>{shorten(profile.address)} <span>▣</span></button><p>{profile.bio || "No bio yet."}</p></div>{isOwnProfile && <button className="outline-button" onClick={onEditProfile}>Edit profile</button>}</section><section className="profile-stats"><div><span>Tokens created</span><strong>{profile.created?.length || 0}</strong></div><div><span>Proto holdings</span><strong>{profile.holdings?.length || 0}</strong></div><div><span>Member since</span><strong>Today</strong></div></section>{isOwnProfile && <CreatorFeeClaim address={user} txClient={txClient} onConnect={onConnect} />}<section className="profile-section"><div className="section-heading"><h2>Tokens created</h2><span>{profile.created?.length || 0}</span></div><div className="token-grid compact-grid">{(profile.created?.length ? profile.created : []).map((token) => <TokenCard token={token} onOpen={onOpen} key={token.token} />)}{!profile.created?.length && <div className="empty-state">Your launched tokens will appear here.</div>}</div></section><section className="profile-section"><div className="section-heading"><h2>Holdings</h2><span>{profile.holdings?.length || 0}</span></div><div className="token-grid compact-grid">{(profile.holdings || []).map((token) => <TokenCard token={token} onOpen={onOpen} key={token.token} />)}</div></section>{!connectedAddress && <button className="primary-button" onClick={onConnect}>Connect to see your profile</button>}</main>;
}
...
function App() {
  ...
  const page = useMemo(() => { if (route.startsWith("#/create")) return <Create address={address} txClient={txClient} onConnect={connect} onCreated={() => { window.location.hash = "#/explore"; }} />; if (route.startsWith("#/token/")) return <Trade tokenAddress={route.split("/")[2]} txClient={txClient} onOpenProfile={openProfile} onConnect={connect} />; if (route.startsWith("#/profile")) return <Profile viewAddress={route.split("/")[2] || address} connectedAddress={address} txClient={txClient} onConnect={connect} onOpen={open} onEditProfile={() => setProfilePrompt(true)} />; return <Explore onOpen={open} onCreate={() => { window.location.hash = "#/create"; }} />; }, [route, address, txClient]);
